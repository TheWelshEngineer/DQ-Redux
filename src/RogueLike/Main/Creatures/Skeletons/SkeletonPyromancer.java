...
public class SkeletonPyromancer extends Creature{
	...
	private static final long serialVersionUID = -8488276225093522062L;
	...
	public SkeletonPyromancer( Creature player, String name, char glyph, Color color, int id, int depth) {
		...
		this.setID(id);
		new SkeletonPyromancerAI(this, player);
		Item startWeapon = FactoryManager.getItemFactory().newClub(0, false, true, true, false);
		this.inventory().add(startWeapon);
		startWeapon = FactoryManager.getItemFactory().newFireboltWand(0, player, false);
		this.inventory().add(startWeapon);
