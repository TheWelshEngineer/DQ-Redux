...
public class Creature implements Cloneable {
    ...
    public int armorClass() {
        ...
        int armorClassBonuses =
                (weapon == null ? 0 : weapon.armorClass())
                        + (ring == null ? 0 : ring.armorClass())
                        + (shield == null ? 0 : shield.armorClass());
    ...
    public int visionRadius() {
        int returnVisionRadius =
                visionRadius
                        + (weapon == null ? 0 : weapon.visionRadius())
                        + (armor == null ? 0 : armor.visionRadius())
                        + (ring == null ? 0 : ring.visionRadius())
                        + (shield == null ? 0 : shield.visionRadius());
    ...
    public int strength() {
        int returnStrength =
                strength
                        + (weapon == null ? 0 : weapon.strength())
                        + (armor == null ? 0 : armor.strength())
                        + (ring == null ? 0 : ring.strength())
                        + (shield == null ? 0 : shield.strength());
        if (affectedBy(Effect.giantStrength) == true) {
            returnStrength += 4;
        }
    ...
    public int dexterity() {
        int returnDexterity =
                dexterity
                        + (weapon == null ? 0 : weapon.dexterity())
                        + (armor == null ? 0 : armor.dexterity())
                        + (ring == null ? 0 : ring.dexterity())
                        + (shield == null ? 0 : shield.dexterity());
    ...
    public int intelligence() {
        int returnIntelligence =
                intelligence
                        + (weapon == null ? 0 : weapon.intelligence())
                        + (armor == null ? 0 : armor.intelligence())
                        + (ring == null ? 0 : ring.intelligence())
                        + (shield == null ? 0 : shield.intelligence());
    ...
    public int saveBonus(DamageType damageType) {
        ...
        switch (damageType) {
            case ACID:
                wardBonus = affectedBy(Effect.causticWard) ? 5 : 0;
                break;
            case CHAOS:
                wardBonus = affectedBy(Effect.eldritchWard) ? 5 : 0;
                break;
            case FIRE:
                wardBonus = affectedBy(Effect.magmaWard) ? 5 : 0;
                break;
            case FROST:
                wardBonus = affectedBy(Effect.chillWard) ? 5 : 0;
                break;
            case MAGIC:
                wardBonus = affectedBy(Effect.arcaneWard) ? 5 : 0;
                break;
            case PHYSICAL:
                wardBonus = affectedBy(Effect.bladeWard) ? 5 : 0;
                break;
            case POISON:
                wardBonus = affectedBy(Effect.venomousWard) ? 5 : 0;
                break;
            case SHOCK:
                wardBonus = affectedBy(Effect.arcWard) ? 5 : 0;
                break;
            case TRUE:
                wardBonus = 0;
                break;
            default:
                throw new IllegalArgumentException(damageType.toString());
        }
    ...
    public DamageType unarmedDamageType() {
        // TODO: I've removed the ability for equipment to override the base unarmed damage type
        // here. We may want
    ...
    public String details() {
        return String.format(
                "   Level: %d   Health: %s   Armor: %s", level, healthAsString(), armorAsString());
    }
    ...
    public String talkToSelf() {
        switch (ExtraMaths.diceRoll(1, 8)) {
            case 1:
                return "Ti sbrn.. Hsi oig.";
            case 2:
                return "Uh-gaddt gig ur uyaan.";
            case 3:
                return "Ir'gymn uh.";
            case 4:
                return "Icudg o ugsbe rudnw ol ofrafnu eraon o.";
            case 5:
                return "Iwne Fgzai aigse oih? Odri az smkn twtngt.";
            case 6:
                return "Hco, oo dialn og!";
            case 7:
                return "Enhro etshkicnogr btu.";
            case 8:
                return "U tijsanh? T agcd epiolhv, rua.";
            default:
                return "Ti sbrn.. Hsi oig.";
        }
    }
    ...
    public String talkToOther() {
        switch (ExtraMaths.diceRoll(1, 8)) {
            case 1:
                return "Mwi ae? Sakttt Fgzai ngagi?";
            case 2:
                return "Oge sw? Hothehhcl etl! Aow.";
            case 3:
                return "Idc ey? Sahlopu ledw.";
            case 4:
                return "Ym mhsee'aytt, aht..";
            case 5:
                return "Awepewo nsrsheo, guglidb gte.";
            case 6:
                return "Wa? Itta! Htw hsaatw?";
            case 7:
                return "Ihntge? Mdoisd ryaoeuh.";
            case 8:
                return "Hycotua wt atkser if.";
            default:
                return "Mwi ae? Sakttt Fgzai ngagi?";
        }
    }
    ...
    public void attack(Creature other) {
        ...
        int attackBonus = 0;
        if (weapon != null) {
            if ((this.weapon.isSimple() && this.simpleWeaponsLevel() >= 2)
                    || (this.weapon.isMartial() && this.martialWeaponsLevel() >= 2)
                    || (this.weapon.isFinesse() && this.finesseWeaponsLevel() >= 2)) {
            ...
            if (weapon.isVersatile() && shield == null) {
                amount +=
                        (int) (weapon.versatileDamageDice().roll())
                                + attackBonus
                                + weapon.upgradeLevel();
            } else {
        ...
        if (this.weapon() != null
                && ((this.weapon.isSimple() && this.simpleWeaponsLevel() >= 1)
                        || (this.weapon.isMartial() && this.martialWeaponsLevel() >= 1)
                        || (this.weapon.isFinesse() && this.finesseWeaponsLevel() >= 1))) {
        ...
        if (attackRoll >= other.armorClass() || attackRoll >= 20) {
            ...
            if (weapon != null) {
                other.damage(
                        damage,
                        String.format("Killed by a %s using a %s", this.name, this.weaponName()));
                    ...
                    other.addEffect(
                            (Effect)
                                    this.ai()
                                            .factory
                                            .effectFactory
                                            .paralysed(this.proficiencyBonus())
                                            .clone());
                } else if (this.weapon.isFinesse()
                        && this.finesseWeaponsLevel() >= 3
                        && attackRoll >= 20) {
                    other.addEffect(
                            (Effect)
                                    this.ai()
            ...
            if (this.affectedBy(Effect.electrocharged)) {
                if (weapon != null) {
                    other.damage(
                            electroDamage,
                            String.format(
                                    "Killed by a %s using a %s", this.name, this.weaponName()));
                } else {
            ...
            if (weapon != null && weapon.enchantment() != null && other.hp() >= 1) {
                if (weapon.enchantment()
                        .checkActivation(weapon.upgradeLevel() * this.intelligenceModifier())) {
            ...
            if (other.armor() != null && other.armor().enchantment() != null && other.hp() >= 1) {
                if (other.armor()
                        .enchantment()
                        .checkActivation(
                                other.armor().upgradeLevel() * other.intelligenceModifier())) {
            ...
            if (other.shield() != null && other.shield().enchantment() != null && other.hp() >= 1) {
                if (other.shield()
                        .enchantment()
                        .checkActivation(
                                other.shield().upgradeLevel() * other.intelligenceModifier())) {
    ...
    private void throwAttack(Item item, Creature other) {
        ...
        if (item.isThrownWeapon()) {
            if ((item.isSimple() && this.simpleWeaponsLevel() >= 2)
                    || (item.isMartial() && this.martialWeaponsLevel() >= 2)
                    || (item.isFinesse() && this.finesseWeaponsLevel() >= 2)) {
                amount += this.proficiencyBonus();
            }
        ...
        Damage damage =
                new Damage(
                        amount, false, item.effectiveDamageType(), factory().effectFactory, true);
        ...
        if ((item.isSimple() && this.simpleWeaponsLevel() >= 1)
                || (item.isMartial() && this.martialWeaponsLevel() >= 1)
                || (item.isFinesse() && this.finesseWeaponsLevel() >= 1)) {
            attackRoll += this.proficiencyBonus();
        }
        ...
        if (attackRoll >= other.armorClass() || attackRoll >= 20) {
            // doAction("throw a %s at the %s for %d damage", nameOf(item), other.name,
            // damage.amount());
            ...
            other.damage(
                    damage, String.format("Killed by a %s using a %s", this.name, item.name()));
            ...
            if (this.affectedBy(Effect.electrocharged)) {
                other.damage(
                        electroDamage,
                        String.format("Killed by a %s using a %s", this.name, item.name()));
            }
            if (other.hp > 0) {
                this.setLastTarget(other);
            }
            ...
            if (item.isThrownWeapon() && item.enchantment() != null && other.hp() >= 1) {
                if (item.enchantment()
                        .checkActivation(item.upgradeLevel() * this.intelligenceModifier())) {
    ...
    public void rangedWeaponAttack(Creature other) {
        ...
        if (weapon != null) {
            ...
            amount +=
                    (int) (weapon.rangedDamageDice().roll()) + attackBonus + weapon.upgradeLevel();
        } else {
        ...
        if (attackRoll >= other.armorClass() || attackRoll >= 20) {
            // doAction("fire the %s at the %s for %d damage", nameOf(weapon), other.name,
            // damage.amount());
    ...
    public void pickup() {
        ...
        if ((inventory.isFull() && !item.isGold()) || item == null) {
            ...
            stackItems();
            if (item.isThrownWeapon()) {
                if (item.getWasCreatureWepon()
                        && this.weapon() == null
                        && (item.getOwner().equals(this))) {
        ...
        if (item.isWeapon()) {
            if (item == weapon) {
                ...
                if (weapon != null && weapon.curse() != null) {
                    notify("Your " + nameOf(weapon) + " is cursed! You can't let go of it!");
                } else if ((item.usesStrength() && this.strength() < item.strengthRequirement())
                        || (item.usesDexterity() && this.dexterity() < item.dexterityRequirement())
                        || (item.usesIntelligence()
                                && this.intelligence() < item.intelligenceRequirement())) {
                    notify("You aren't skilled enough to use the " + nameOf(item) + ".");
                } else if (item.isTwoHanded() && shield != null) {
                    notify(
                            "The "
                                    + nameOf(item)
                                    + " is too unwieldy to use alongside your "
                                    + nameOf(shield)
                                    + "!");
                } else {
                    if (item.curse() != null) {
                        if (item.isCurseKnown()) {
                            ...
                            notify(
                                    "Your senses warn you of a foul magic lurking within the "
                                            + nameOf(item)
                                            + ".");
                            notify("The " + nameOf(item) + " is cursed!");
            ...
            if (item == armor) {
                ...
                if (armor != null && armor.curse() != null) {
                    notify("Your " + nameOf(armor) + " is cursed! You can't take it off!");
                } else if ((item.isMediumArmor() && this.armorTrainingLevel() < 1)
                        || (item.isHeavyArmor() && this.armorTrainingLevel() < 2)) {
                    notify("You aren't skilled enough to use the " + nameOf(item) + ".");
                } else {
                    if (item.curse() != null) {
                        if (item.isCurseKnown()) {
            ...
            if (item == shield) {
                ...
                if (shield != null && shield.curse() != null) {
                    notify("Your " + nameOf(shield) + " is cursed! You can't put it down!");
                } else if (((this.armorTrainingLevel() < 1)
                        || (item.isTowerShield() && this.armorTrainingLevel() < 2))) {
                    notify("You aren't skilled enough to use the " + nameOf(item) + ".");
        ...
        switch (slot) {
            case 1:
                ...
                if (quickslot_1().isThrownWeapon()) {
                    ...
                    if (quickslot_1().writtenSpells().size() == 1
                            && !quickslot_1().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_1().writtenSpells().get(0).name()),
                                quickslot_1().writtenSpells().get(0),
                                quickslot_1());
                    } else if (quickslot_1().writtenSpells().size() == 1
                            && quickslot_1().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
                                .playerAICastSpell(
                                        quickslot_1(),
            ...
            case 2:
                ...
                if (quickslot_2().isThrownWeapon()) {
                    ...
                    if (quickslot_2().writtenSpells().size() == 1
                            && !quickslot_2().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_2().writtenSpells().get(0).name()),
                                quickslot_2().writtenSpells().get(0),
                                quickslot_2());
                    } else if (quickslot_2().writtenSpells().size() == 1
                            && quickslot_2().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
            ...
            case 3:
                ...
                if (quickslot_3().isThrownWeapon()) {
                    ...
                    if (quickslot_3().writtenSpells().size() == 1
                            && !quickslot_3().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_3().writtenSpells().get(0).name()),
                                quickslot_3().writtenSpells().get(0),
                                quickslot_3());
                    } else if (quickslot_3().writtenSpells().size() == 1
                            && quickslot_3().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
            ...
            case 4:
                ...
                if (quickslot_4().isThrownWeapon()) {
                    ...
                    if (quickslot_4().writtenSpells().size() == 1
                            && !quickslot_4().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_4().writtenSpells().get(0).name()),
                                quickslot_4().writtenSpells().get(0),
                                quickslot_4());
                    } else if (quickslot_4().writtenSpells().size() == 1
                            && quickslot_4().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
            ...
            case 5:
                ...
                if (quickslot_5().isThrownWeapon()) {
                    ...
                    if (quickslot_5().writtenSpells().size() == 1
                            && !quickslot_5().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_5().writtenSpells().get(0).name()),
                                quickslot_5().writtenSpells().get(0),
                                quickslot_5());
                    } else if (quickslot_5().writtenSpells().size() == 1
                            && quickslot_5().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
            ...
            case 6:
                ...
                if (quickslot_6().isThrownWeapon()) {
                    ...
                    if (quickslot_6().writtenSpells().size() == 1
                            && !quickslot_6().writtenSpells().get(0).isSelfCast()) {
                        return new SpellTargetingScreen(
                                this,
                                String.format(
                                        "Cast %s at?", quickslot_6().writtenSpells().get(0).name()),
                                quickslot_6().writtenSpells().get(0),
                                quickslot_6());
                    } else if (quickslot_6().writtenSpells().size() == 1
                            && quickslot_6().writtenSpells().get(0).isSelfCast()) {
                        // this.castSpell(quickslot_1().writtenSpells().get(0), this.x(), this.y(),
                        // quickslot_1());
                        this.ai()
                ...
                // TODO update throw, quaff, and read to empty the quickslot, update get to
                // automatically return items to their relevant quickslots
    ...
    public void scaleManaWithDepth(int depth) {
        this.setMaxMana(
                (int) ((int) this.maxMana() + ((depth + 1) * this.scalingFactorWithDepth())));
    }

    public void scaleStrengthWithDepth(int depth) {
        this.setStrength(
                (int) ((int) this.strength() + ((depth + 1) * this.scalingFactorWithDepth())));
    }

    public void scaleDexterityWithDepth(int depth) {
        this.setDexterity(
                (int) ((int) this.dexterity() + ((depth + 1) * this.scalingFactorWithDepth())));
    }

    public void scaleIntelligenceWithDepth(int depth) {
        this.setIntelligence(
                (int) ((int) this.intelligence() + ((depth + 1) * this.scalingFactorWithDepth())));
    }
    ...
    public void modifyFood(int amount) {
        ...
        if (food > maxFood) {
            ...
            Damage overeatDamage =
                    new Damage(2, false, DamageType.TRUE, factory().effectFactory, false);
    ...
    public void stepInPit() {
        if (realTile(this.x(), this.y(), this.z()) == Tile.PIT
                && !this.isFlying()
                && !affectedBy(Effect.levitating)) {
    ...
    public void stepInFire() {
        if (realSubtile(this.x(), this.y(), this.z()) == Tile.FIRE
                && !this.isFlying()
                && !affectedBy(Effect.levitating)
                && (ExtraMaths.d10() > 4)) {
    ...
    public void stepInParalysis() {
        if (realGastile(this.x(), this.y(), this.z()) == Tile.PARALYZE_GAS
                && (ExtraMaths.d10() > 4)) {
    ...
    public void stepInConfusion() {
        if (realGastile(this.x(), this.y(), this.z()) == Tile.CONFUSE_GAS
                && (ExtraMaths.d10() > 4)) {
    ...
    public void applyCurses() {
        ...
        if (armor != null && armor.curse() != null) {
            if (armor.curse()
                    .checkActivation(-(armor.upgradeLevel() * this.intelligenceModifier()))) {
        ...
        if (shield != null && shield.curse() != null) {
            if (shield.curse()
                    .checkActivation(-(shield.upgradeLevel() * this.intelligenceModifier()))) {
        ...
        if (ring != null && ring.curse() != null) {
            if (ring.curse()
                    .checkActivation(-(ring.upgradeLevel() * this.intelligenceModifier()))) {
        ...
        if (!isDead) {
            for (Creature other : getCreaturesWhoSeeMe()) {
                if (other == this) {
                    ...
                    other.notify(
                            String.format("The %s %s.", name, makeThirdPerson(message)), params);
                }
            }
        }
    }
    ...
    public void castSpell(Spell spell, int x2, int y2, Item item) {
        ...
        if (spell.castOnTile()) {
            ...
            if (item != null) {
                if ((item.usesStrength() && this.strength() < item.strengthRequirement())
                        || (item.usesDexterity() && this.dexterity() < item.dexterityRequirement())
                        || (item.usesIntelligence()
                                && this.intelligence() < item.intelligenceRequirement())) {
                    notify(
                            "The spell housed within the "
                                    + nameOf(item)
                ...
                Effect sneak =
                        (Effect)
                                this.ai()
                                        .factory
                                        .effectFactory
                                        .invisible(this.proficiencyBonus())
                                        .clone();
