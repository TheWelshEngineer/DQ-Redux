...
public class CreatureAI {
    ...
    public void onEnter(int x, int y, int z, Tile tile) {
        ...
        if ((tile.isOpen() && !tile.isBars()) || (tile.isStairs() && !tile.isBars())) {
            if (tile == Tile.PIT
                    && (!creature.affectedBy(Effect.levitating) && !creature.isFlying())) {
    ...
    public void wander() {
        ...
        if ((other != null && other.glyph() == creature.glyph())
                || (other != null && other.isContainer() == true)) {
    ...
    public boolean canSee(int wx, int wy, int wz) {
        ...
        if ((creature.x - wx) * (creature.x - wx) + (creature.y - wy) * (creature.y - wy)
                > creature.visionRadius() * creature.visionRadius()) return false;

        for (Point p : new Line(creature.x, creature.y, wx, wy)) {
            // fixed
            if ((creature.realTile(p.x, p.y, wz).isGround()
                            && !creature.realTile(p.x, p.y, wz).isObscuring())
                    || creature.realTile(p.x, p.y, wz).isStairs()
                    || p.x == wx && p.y == wy) continue;
    ...
    public boolean isAdjacentTo(Creature target) {
        ...
        if (((target.x >= 0 && target.x <= creature.x + 1)
                        || (target.x <= 0 && target.x >= creature.x - 1))
                && ((target.y >= 0 && target.y <= creature.y + 1)
                        || (target.y <= 0 && target.y >= creature.y - 1))) {
            return true;
        }
        return false;
    ...
    protected boolean canRangedWeaponAttack(Creature other) {
        return creature.weapon() != null
                && creature.weapon().rangedDamageDice() != null
    ...
    protected Item getWeaponToThrow() {
        ...
        for (Item item : creature.inventory().getItems()) {
            if (item == null
                    || item.thrownDamageDice() != null
                    || creature.weapon() == item
                    || creature.armor() == item
                    || creature.shield() == item
                    || creature.ring() == item) {
                continue;
            }
    ...
    protected boolean canUseBetterEquipment() {
        ...
        for (Item item : creature.inventory().getItems()) {
            ...
            if (!isArmor && itemWeaponRating > currentWeaponRating
                    || isArmor && item.armorClass() > currentArmorRating) {
                return true;
            }
        }
        return false;
