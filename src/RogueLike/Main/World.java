...
public class World {
    ...
    public Player player() {
        if (player == null) {
            // defensive programming! we don't want things to pick up a dead reference to the
            // player.
            // anything that needs a reference should only need to get one after worldgen is
            // complete
    ...
    public List<Integer> specialDepths() {
        return specialDepths;
    }
    ...
    public Tile tile(int x, int y, int z) {
        if (x < 0 || x >= width || y < 0 || y >= height || z < 0 || z >= depth) return Tile.BOUNDS;
        else return tiles[x][y][z];
    }

    public Tile subtile(int x, int y, int z) {
        if (x < 0 || x >= width || y < 0 || y >= height || z < 0 || z >= depth) return Tile.BOUNDS;
        else return subtiles[x][y][z];
    }

    public Tile gastile(int x, int y, int z) {
        if (x < 0 || x >= width || y < 0 || y >= height || z < 0 || z >= depth) return Tile.BOUNDS;
        else return gastiles[x][y][z];
    }
    ...
    public void addPlayer(
            NotificationHistory notificationHandle, PlayerBuildDetails playerDetails) {
        if (player != null) {
            throw new IllegalStateException("Player already exists!");
        }
        player =
                factory.creatureFactory.newPlayer(
                        new FieldOfView(this), notificationHandle, playerDetails);
    ...
    public Point getEmptyLocationForTrap(int depth) {
        ...
        do {
            x = (int) (Math.random() * width);
            y = (int) (Math.random() * height);
        } while (tile(x, y, depth).noItems()
                || tile(x, y, depth).isBars()
                || creature(x, y, depth) != null
                || item(x, y, depth) != null);
    ...
    public boolean addAtEmptySpace(Item item, int x, int y, int z) {
        ...
        while (!points.isEmpty()) {
            ...
            if (items[p.x][p.y][p.z] == null
                    && !tile(p.x, p.y, p.z).isStairs()
                    && !tile(p.x, p.y, p.z).noItems()) {
    ...
    public void updateOnCurrentFloor(Creature player) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (subtiles[x][y][player.z()] != null
                        && subtiles[x][y][player.z()].isDecaying()
                        && ExtraMaths.d10() > 9) {
                    changeSubTile(x, y, player.z(), null);
                }
                if (gastiles[x][y][player.z()] != null
                        && gastiles[x][y][player.z()].isDecaying()
                        && ExtraMaths.d10() > 9) {
