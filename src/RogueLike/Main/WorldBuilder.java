...
public class WorldBuilder {
    ...
    public WorldBuilder(int width, int height, int depth) {
        ...
        this.structureReservedTiles =
                new boolean[width][height][depth]; // defaults to filled with False
        this.nextRegion = 1;
    }
    ...
    public void markDepthAsSpecial(int depth) {
    ...
    public World build(NotificationHistory playerNotifications, PlayerBuildDetails playerDetails) {
        ...
        // Structures do their generation before blueprints, as structures are intended to have more
        // specific
        // generation, and we don't want that to be stepped on by the more generic generation from
        // blueprints
        ...
        // TODO: move the victory item to a VictoryFloor blueprint? or make it drop from the final
        // boss?
        ...
        while (!open.isEmpty()) {
            ...
            for (Point neighbor : p.neighbors8()) {
                ...
                if (regions[neighbor.x][neighbor.y][neighbor.z] > 0
                        || tiles[neighbor.x][neighbor.y][neighbor.z] == Tile.WALL) continue;
    ...
    private void connectRegionsDown(int z, int r1, int r2) {
        List<Point> candidates =
                findRegionOverlaps(z, r1, r2).stream()
                        .filter(
                                p ->
                                        !isStructureTileReserved(
                                                p)) // filter out any points that are in a reserved
                        // area
        ...
        if (candidates.isEmpty()) {
            throw new WorldGenerationException(
                    String.format(
                            "Failed to generate world - could not generate downwards stairs on"
                                    + " depth %d",
                            z));
        }
    ...
    private WorldBuilder addExitStairs() {
        ...
        } while (tiles[x][y][0] != Tile.FLOOR // exit stairs must only be placed on the floor
                || structureReservedTiles[x][y][
                        0] // exit stairs must not be placed within a structure's reserved area
        );
    ...
    /**
     * Change all bars tiles to the appropriate one, depending on where other adjacent bars tiles
     * are.
     */
    ...
    private WorldBuilder addAllBlueprints() {
        for (int z = 0; z < depth; z++) {
            switch (z) {
                case 5:
                case 10:
                    addBlueprint(new MerchantFloor(this, z));
                    break;
                default:
                    addBlueprint(new CaveFloor(this, z));
                    break;
            }
        }
        return this;
