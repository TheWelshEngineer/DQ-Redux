...
public class ChooseAbilityScreen implements Screen {
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
        ...
        terminal.writeCenter(
                String.format(
                        "%c %c Strength ( %d / %s ) %c %c",
                        strDown,
                        strLeft,
                        playerStrength,
                        ExtraMaths.modifierToString(strengthModifier()),
                        strRight,
                        strUp),
                y += 3);
        terminal.writeCenter(
                String.format(
                        "%c %c Dexterity ( %d / %s ) %c %c",
                        dexDown,
                        dexLeft,
                        playerDexterity,
                        ExtraMaths.modifierToString(dexterityModifier()),
                        dexRight,
                        dexUp),
                y += 2);
        terminal.writeCenter(
                String.format(
                        "%c %c Intelligence ( %d / %s ) %c %c",
                        intDown,
                        intLeft,
                        playerIntelligence,
                        ExtraMaths.modifierToString(intelligenceModifier()),
                        intRight,
                        intUp),
                y += 2);
        if (check == 0) {
            ...
            terminal.writeCenter(
                    "Strength increases the damage dealt by simple and martial weapons,", y += 1);
            terminal.writeCenter(
                    "as well as the amount of health you gain from levelling up.", y += 1);
        } else if (check == 1) {
            terminal.writeCenter("Dexterity is a measure of your agility and reflexes.", y += 6);
            terminal.writeCenter(
                    "Dexterity increases the damage dealt by ranged and finesse weapons,", y += 1);
            terminal.writeCenter("as well as providing a bonus to light and medium armor.", y += 1);
        } else if (check == 2) {
            terminal.writeCenter(
                    "Intelligence represents your willpower and magical talent.", y += 6);
            terminal.writeCenter("Intelligence increases the effectiveness of wands,", y += 1);
            terminal.writeCenter(
                    "as well as the amount of mana you gain from levelling up.", y += 1);
        }
        ...
        terminal.writeCenter(
                String.format(
                        "-- [%s / %s]: Move Selection | [%s / %s]: Increase/Decrease Attribute --",
    ...
    public Screen respondToUserInput(KeyEvent key) {
        ...
        switch (key.getKeyCode()) {
            ...
            case KeybindManager.navigateMenuConfirm:
                if (abilityPoints < 1) {
                    return new ChooseSkillScreen(
                            playerClass, playerAbilities, playerName, playerAncestry);
                } else {
                    return this;
                }

            case KeybindManager.navigateMenuBack:
                return new ChooseClassScreen(playerAncestry);
        }
        return this;
    }
    ...
    public String randomLastName() {
        switch (ExtraMaths.diceRoll(1, 40)) {
            ...
            case 36:
                return "the Bold";
            case 37:
                return "the Aledrinker";
            case 38:
                return "the Always-Hungry";
            case 39:
                return "the Foolish";
            case 40:
                return "the Ironhearted";
            default:
                return "Gwallawg";
        }
    }
