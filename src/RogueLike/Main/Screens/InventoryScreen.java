            ...
            if (i < inventory.getItems().size()) { // spellbook.get(0).name()
                if (checkIfSelected(i, check)) {
                    ...
                    if (item.enchantment() != null
                            ...
                            && item.isWeapon()
                            && item.isIdentified()
                            && (item.defaultName().length() > 8)
                            && item == player.weapon()) {
                        itemDetails =
                                String.format(
                                        ">> %c %s%cx%d%s",
                                        inventory.get(i).glyph(),
                                        player.nameOf(inventory.get(i)),
                                        (char) 255,
                                        inventory.get(i).stackAmount(),
                                        equipped,
                                        check);
                    }
                    CharSequence nonBreakingSpace =
                            (CharSequence) (String.format("%c", (char) 255));
                    ...
                    int x = 42;
                    terminal.write(
                            String.format(">> %c %s", item.glyph(), player.nameOf(item)), x, z);
                    ...
                    if (item.isWeapon()) {
                        if (item.isIdentified()) {
                            ...
                            if (item.isVersatile()) {
                                damage =
                                        String.format(
                                                "%s%s%d / %s%s%d",
                                                item.damageDice().toString(),
                                                symbol,
                                                bonus,
                                                item.versatileDamageDice().toString(),
                                                symbol,
                                                bonus);
                            } else {
                                damage =
                                        String.format(
                                                "%s%s%d",
                                                item.damageDice().toString(), symbol, bonus);
                            }
                            ...
                            if (item.thrownDamageDice() != null
                                    && item.thrownDamageDice() != Dice.d1) {
                                damage =
                                        String.format(
                                                "%s%s%d",
                                                item.thrownDamageDice().toString(), symbol, bonus);
                            ...
                            if (item.rangedDamageDice() != null) {
                                ...
                                damage =
                                        String.format(
                                                "%s%s%d",
                                                item.rangedDamageDice().toString(), symbol, bonus);
                            ...
                            if (item.usesStrength()) {
                                attackbonus =
                                        item.upgradeLevel()
                                                + player
                                                        .strengthModifier() /*+ player.skillSimpleWeapons()*/;
                            }
                            if (item.usesIntelligence()) {
                                attackbonus =
                                        item.upgradeLevel()
                                                + player
                                                        .intelligenceModifier() /*+ player.skillMartialWeapons()*/;
                            }
                            if (item.usesDexterity()) {
                                attackbonus =
                                        item.upgradeLevel()
                                                + player
                                                        .dexterityModifier() /*+ player.skillFinesseWeapons()*/;
                            }
                            ...
                            terminal.write(
                                    String.format("Accuracy Bonus: %s%d", symbol, attackbonus),
                                    x,
                                    z++);
                            terminal.write(
                                    String.format("Damage Type: %s", item.effectiveDamageType()),
                                    x,
                                    z++);
                            ...
                            if (item.usesStrength()) {
                                attackbonus =
                                        player.strengthModifier() /*+ player.skillSimpleWeapons()*/;
                            }
                            if (item.usesIntelligence()) {
                                attackbonus =
                                        player
                            ...
                            if (item.usesDexterity()) {
                                attackbonus =
                                        player
                                                .dexterityModifier() /*+ player.skillRangedWeapons()*/;
                            }
                            symbol = "";
                            if (attackbonus >= 0) {
                                symbol = "+";
                        ...
                        if (!traits.isEmpty()) {
                            ...
                            ArrayList<String> traits_lines =
                                    TextUtils.joinStringsWithLineBreaks(traits, ", ", maxLength);
                    ...
                    if (item.isArmor() || item.isShield()) {
                        ...
                        if (item.isIdentified()) {
                            armor =
                                    String.format(
                                            "Armor Class: %d",
                                            item.armorClass() + item.upgradeLevel());
                        } else {
                    ...
                    if (item.strengthRequirement() > 0
                            || item.dexterityRequirement() > 0
                            || item.intelligenceRequirement() > 0) {
                        String skill = "";
                        ...
                        if (item.usesIntelligence()) {
                            skill =
                                    String.format(
                                            "Intelligence (%d)", item.intelligenceRequirement());
                        }
                    ...
                    if (item.isIdentified() || (item.foodValue() > 0 || item.isIronKey())) {
                        terminal.write(
                                String.format("Value: %s gold", item.currentGoldValue()), x, z++);
                    } else {
                        terminal.write(
                                String.format("Value: %s gold (?)", item.baseGoldValue()), x, z++);
                    }
                    z++;
                    if (item.quaffEffect() != null) {
                        if (item.isIdentified()) {
                            terminal.write(
                                    item.getDescription().getPotionDescriptionBase(), x, z++);
                            terminal.write(
                                    item.getDescription().getPotionDescriptionKnown(), x, z++);
                        } else {
                            terminal.write(
                                    item.getDescription().getPotionDescriptionBase(), x, z++);
                            terminal.write(
                                    item.getDescription().getPotionDescriptionUnknown(), x, z++);
                        }
                    }
                    if (item.isWand()) {
                        if (item.isIdentified()) {
                            ...
                            terminal.write(item.getDescription().getWandDescriptionBase(), x, z++);
                            terminal.write(
                                    item.getDescription().getWandDescriptionUnknown(), x, z++);
                        }
                    }
                    ...
                    String equipped = "";
                    if (inventory.get(i) == player.weapon()
                            || inventory.get(i) == player.armor()
                            || inventory.get(i) == player.shield()
                            || inventory.get(i) == player.ring()
                            || inventory.get(i) == player.ammunition()) {
                        equipped = " (equipped)";
                        if (inventory.get(i) == player.quickslot_1()) {
                            equipped = " (equipped - QS1)";
                        }
                    ...
                    String itemDetails =
                            String.format(
                                    ...
                                    equipped,
                                    check);
                    if (inventory.get(i).enchantment() != null
                            && inventory.get(i).curse() == null
                            && inventory.get(i).isWeapon()
                            && inventory.get(i).isIdentified()
                            && (inventory.get(i).defaultName().length() > 8)
                            && inventory.get(i) == player.weapon()) {
                        itemDetails =
                                String.format(
                                        "%c %s%cx%d%s",
                                        inventory.get(i).glyph(),
                    ...
                    if (itemDetails.contains(nonBreakingSpace)) {
                        ...
                        terminal.write(detailsList[0], 5, y);
                        terminal.write(
                                String.format("%c", inventory.get(i).glyph()),
                                5,
                                y++,
                                inventory.get(i).color());
                        terminal.write(String.format("  %s", detailsList[1]), 5, y);
                        y++;
                    } else {
                        terminal.write(itemDetails, 5, y);
                    ...
                    terminal.writeCenter(
                            String.format(
                                    ...
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionEquipItem)),
                            36);
                }
                if (checkIfSelected(i, check) && inventory.get(i).foodValue() > 0) {
                    terminal.writeCenter(
                            String.format(
                                    "-- [%s]: Eat --",
                                    KeybindManager.keybindText(KeybindManager.interactionEatFood)),
                            36);
                }
                if (checkIfSelected(i, check) && inventory.get(i).quaffEffect() != null) {
                    terminal.writeCenter(
                            String.format(
                                    "-- [%s]: Quaff --",
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionDrinkPotion)),
                            36);
                }
                if (checkIfSelected(i, check) && inventory.get(i).writtenSpells().size() > 0) {
                    terminal.writeCenter(
                            String.format(
                                    "-- [%s]: Read --",
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionReadSpell)),
                            36);
                }
                ...
                if (checkIfSelected(i, check) && inventory.get(i).isQuickslottable()) {
                    terminal.writeCenter(
                            String.format(
                                    "-- [%s-%s]: Equip to quickslot 1-6 | [%s]: Drop | [%s]: Throw"
                                            + " --",
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionQuickslot_1),
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionQuickslot_6),
                                    KeybindManager.keybindText(KeybindManager.interactionDropItem),
                                    KeybindManager.keybindText(
                                            KeybindManager.interactionThrowItem)),
                            38);
            ...
            case KeybindManager.interactionReadSpell:
                if (inventory.get(check).writtenSpells().size() > 0) {
                    if (inventory.get(check).isSpellbook()) {
                        player.learnSpell(
                                inventory.get(check).writtenSpells().get(0), inventory.get(check));
                        playScreen.setInputAccepted(true);
                        return null;
                    } else {
                        if (inventory.get(check).writtenSpells().size() == 1
                                && !inventory.get(check).writtenSpells().get(0).isSelfCast()) {
                            return new SpellTargetingScreen(
                                    player,
                                    String.format(
                                            "Cast %s at?",
                                            inventory.get(check).writtenSpells().get(0).name()),
                                    inventory.get(check).writtenSpells().get(0),
                                    inventory.get(check));
                        } else if (inventory.get(check).writtenSpells().size() == 1
                                && inventory.get(check).writtenSpells().get(0).isSelfCast()) {
                            // player.castSpell(inventory.get(check).writtenSpells().get(0),
                            // player.x(), player.y(), inventory.get(check));
                            // playScreen.setInputAccepted(true);
                            player.ai()
                                    .playerAICastSpell(
                                            inventory.get(check),
