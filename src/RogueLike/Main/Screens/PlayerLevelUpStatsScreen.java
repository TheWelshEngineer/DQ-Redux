...
public class PlayerLevelUpStatsScreen implements Screen {
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
        ...
        terminal.writeCenter(
                String.format(
                        "%c %c Strength ( %d / +%d ) %c %c",
                        strDown, strLeft, playerStrength, strMod(), strRight, strUp),
                y += 3);
        terminal.writeCenter(
                String.format(
                        "%c %c Dexterity ( %d / +%d ) %c %c",
                        dexDown, dexLeft, playerDexterity, dexMod(), dexRight, dexUp),
                y += 2);
        terminal.writeCenter(
                String.format(
                        "%c %c Intelligence ( %d / +%d ) %c %c",
                        intDown, intLeft, playerIntelligence, intMod(), intRight, intUp),
                y += 2);
        if (check == 0) {
