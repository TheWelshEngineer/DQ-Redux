...
public class GameplayScreen implements Screen {
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
        ...
        //
        String depth =
                String.format("Depth: %d (%d,%d)", player.z() + 1, player.x() + 1, player.y() + 1);
        ...
        for (Effect effect : effects) {
            if (effect.name() != null) {
                ...
                if (effect.showInMenu()) {
                    terminal.write(
                            String.format("    %s (%d)", effect.name(), effect.duration()), 95, y2);
    ...
    public Screen respondToUserInput(KeyEvent key) {
        ...
        if (subscreen != null) {
            ...
            switch (key.getKeyCode()) {
                    //
                    // Movement Controls
                case KeybindManager.movementWest:
                    player.ai().playerAIMoveWest();
                    break;
                case KeybindManager.movementEast:
                    player.ai().playerAIMoveEast();
                    break;
                case KeybindManager.movementNorth:
                    player.ai().playerAIMoveNorth();
                    break;
                case KeybindManager.movementSouth:
                    player.ai().playerAIMoveSouth();
                    break;
                case KeybindManager.movementNorthWest:
                    player.ai().playerAIMoveNorthWest();
                    break;
                case KeybindManager.movementNorthEast:
                    player.ai().playerAIMoveNorthEast();
                    break;
                case KeybindManager.movementSouthWest:
                    player.ai().playerAIMoveSouthWest();
                    break;
                case KeybindManager.movementSouthEast:
                    player.ai().playerAIMoveSouthEast();
                    break;
                case KeybindManager.movementWait:
                ...
                case KeybindManager.interactionEquipItem:
                    ...
                    break;
                case KeybindManager.interactionPickUpItem:
                    player.ai().playerAIGetItem();
                    break;
                case KeybindManager.interactionExamineItem:
                    subscreen = new ExamineScreen(player);
                    break;
                case KeybindManager.interactionLook:
                    subscreen =
                            new LookScreen(
                                    player,
                                    String.format(
                                            "Use the movement controls to look around, or press"
                                                    + " [%s] to stop looking.",
                                            KeybindManager.keybindText(
                                                    KeybindManager.navigateMenuBack)));
                    break;
                case KeybindManager.interactionThrowItem:
                    subscreen =
                            new ThrowScreen(
                                    player, player.x - getScrollX(), player.y - getScrollY());
                    break;
                ...
                case KeybindManager.interactionQuickslot_1:
                    subscreen =
                            player.useItemFromQuickslot(
                                    1, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionQuickslot_2:
                    subscreen =
                            player.useItemFromQuickslot(
                                    2, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionQuickslot_3:
                    subscreen =
                            player.useItemFromQuickslot(
                                    3, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionQuickslot_4:
                    subscreen =
                            player.useItemFromQuickslot(
                                    4, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionQuickslot_5:
                    subscreen =
                            player.useItemFromQuickslot(
                                    5, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionQuickslot_6:
                    subscreen =
                            player.useItemFromQuickslot(
                                    6, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.interactionSearch:
                    player.ai().playerAISearchArea();
                    break;
                case KeybindManager.interactionFireRangedWeapon:
                    ...
                    } else if (player.ammunition().isArrowAmmunition()
                            && !player.weapon().usesArrowAmmunition()) {
                        player.notify("You don't have the right ammunition ready.");
                    } else if (player.ammunition().isBoltAmmunition()
                            && !player.weapon().usesBoltAmmunition()) {
                        player.notify("You don't have the right ammunition ready.");
                    } else if (player.ammunition().isPowderAmmunition()
                            && !player.weapon().usesPowderAmmunition()) {
                        player.notify("You don't have the right ammunition ready.");
                    } else {
                        subscreen = new RangedWeaponTargetingScreen(player);
                        inputAccepted = true;
                        break;
                    }
                case KeybindManager.interactionLevelUp:
                    if (player.attributePoints() == 0 && player.skillPoints() == 0) {
                        player.notify("You don't have any skill points to spend.");
                        break;
                    } else {
                    ...
                    //
                    // Menu Controls
                case KeybindManager.menuHelp:
                    subscreen = new HelpScreen(false);
                    break;
                case KeybindManager.menuCharacterSheet:
                    subscreen = new CharacterSheetScreen(player);
                    break;
                case KeybindManager.menuIndex:
                    subscreen = new IndexPotionScreen(player, player.factory());
                    break;
                case KeybindManager.menuInventory:
                    subscreen =
                            new InventoryScreen(
                                    this, player, player.x - getScrollX(), player.y - getScrollY());
                    break;
                case KeybindManager.menuActionLog:
                    subscreen = new ActionLogScreen(playerNotifications, world.turnNumber());
                    break;
                    //
                    // case KeyEvent.VK_A: subscreen = new SpellbookScreen(player, player.x -
                    // getScrollX(), player.y - getScrollY(), true); inputAccepted = true; break;
                    //
                    // case KeyEvent.VK_M: subscreen = new FeatbookScreen(player, player.x -
                    // getScrollX(), player.y - getScrollY(), true); inputAccepted = 1; break;
            }
        }
    ...
    private World createWorld(
            NotificationHistory playerNotifications, PlayerBuildDetails playerDetails) {
        // This is a bit of a hacky way of handling the "world fails to generate sometimes" issue -
        // just keep trying
    ...
    private void displayTiles(ExtendedAsciiPanel terminal, int left, int top) {
        ...
        for (int x = 0; x < screenWidth; x++) {
            for (int y = 0; y < screenHeight; y++) {
                ...
                //
                // if(player.magicMapping() > 0 && player.item(wx, wy, player.z) != null &&
                // player.item(wx, wy, player.z).isTrap() > 0 && player.item(wx, wy,
                // player.z).isFound() == 0) {
                // player.item(wx, wy, player.z).changeColor(player.item(wx, wy,
                // player.z).defaultColor());
                // player.item(wx, wy, player.z).changeGlyph(player.item(wx, wy,
                // player.z).defaultGlyph());
                // player.item(wx, wy, player.z).modifyIsFound(1);
                // }
                //
                if (player.canSee(wx, wy, player.z))
                    terminal.write(
                            world.glyph(wx, wy, player.z), x, y, world.color(wx, wy, player.z));
                else terminal.write(fov.tile(wx, wy, player.z).glyph(), x, y, Color.darkGray);
            }
        }
    }
}
