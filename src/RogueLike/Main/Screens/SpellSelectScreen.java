...
public class SpellSelectScreen implements Screen {
    ...
    private ArrayList<String> getList() {
        ...
        for (int i = 0; i < item.writtenSpells().size(); i++) {
            ...
            String line =
                    letters.charAt(i) + " - " + spell.name() + " (" + spell.manaCost() + " mana)";
    ...
    protected Screen use(Spell spell, Item item) {
        if (spell.isSelfCast()) {
            ...
            player.ai()
                    .playerAICastSpell(item, item.writtenSpells().get(0), player.x(), player.y());
            return null;
        } else {
            // return null;
            return new SpellTargetingScreen(
                    player, String.format("Cast %s at?", spell.name()), spell, item);
        }
