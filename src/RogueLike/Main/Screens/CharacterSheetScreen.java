...
public class CharacterSheetScreen implements Screen {
    ...
    public CharacterSheetScreen(Player player) {
        ...
        elements.add( // column 4: damage modifiers
                Arrays.stream(DamageType.RESISTABLE_TYPES)
                        .map(type -> new DamageModifierElement(player, type))
                        .collect(Collectors.toList()));
    }
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
        ...
        terminal.writeMultiline(
                TextUtils.wordWrap(selectedElement.details(), maxWidth, 4), column_x[0], 31);
    ...
    private void moveSelectionVertical(int direction) {
        ...
        if (currentColumn.isEmpty()) {
            throw new IllegalStateException(
                    String.format("Empty column at cX=%d shouldn't be selected.", cX));
        }
        ...
        //  - moveSelectionHorizontal checks that the column it's moving into has a selectable
        // element.
        do {
