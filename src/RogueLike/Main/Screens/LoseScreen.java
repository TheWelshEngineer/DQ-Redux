...
public class LoseScreen implements Screen {
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
        ...
        terminal.writeCenter(
                player.causeOfDeath() + String.format(" on depth %d.", player.z() + 1), 7);
        terminal.writeCenter(
                String.format(
                        "Level %d %s %s | Score: %d | Max Depth Reached: %d",
                        player.level(),
                        player.playerAncestry(),
                        player.playerClass(),
                        player.score(),
                        player.maxDepth() + 1),
                9);
        ...
        terminal.writeCenter(
                String.format(
                        "-- [%s]: Restart your Adventure | [%s]: Export Scores to Desktop --",
                        KeybindManager.keybindText(KeybindManager.navigateMenuConfirm),
                        KeybindManager.keybindText(KeybindManager.navigateMenuFunction_1)),
                24);
    ...
    public Screen respondToUserInput(KeyEvent key) {
        switch (key.getKeyCode()) {
            case KeybindManager.navigateMenuFunction_1:
                ...
                String filename =
                        String.format("%sDwarfQuest %s %s.txt", desktopPath, playerName, localDate);
                try {
                ...
                try {
                    ...
                    writeScores.write(
                            "\n"
                                    + player.causeOfDeath()
                                    + String.format(" on depth %d.", player.z() + 1));
                    writeScores.write(
                            String.format(
                                    "\nLevel %d %s | Score: %d | Max Depth Reached: %d",
                                    player.level(),
                                    player.playerClass(),
                                    player.score(),
                                    player.maxDepth() + 1));
