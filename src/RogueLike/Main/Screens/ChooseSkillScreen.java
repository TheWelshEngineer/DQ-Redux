		...
		for (Skill skill: Skill.values()) {
			...
			terminal.write(
				String.format(
					...
					leftMarker,
					skill,
					booleanToChar(skillsChosen.contains(skill) || classSkills.contains(skill)),
					rightMarker,
					borderVertical
				),
				4,
		...
		case KeybindManager.navigateMenuConfirm: 
			if(skillPoints < 1) {
				...
				return new Zone1Screen(
					new PlayerBuildDetails(playerClass, playerAbilities, skillset, playerName, playerAncestry)
				);
			}else {
				return this;
			}
