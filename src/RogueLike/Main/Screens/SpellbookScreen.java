...
public class SpellbookScreen implements Screen {
    ...
    public void displayOutput(ExtendedAsciiPanel terminal) {
            ...
            if (i < spellbook.getSpells().size()) { // spellbook.get(0).name()
                if (checkIfSelected(i, check)) {
                    terminal.writeCenter(
                            String.format(
                                    "> %s (%d mana) <",
                                    spellbook.get(i).name(), spellbook.get(i).manaCost(), check),
                            y++);
                } else {
                    terminal.writeCenter(
                            String.format(
        ...
        terminal.writeCenter(
                "-- [UP / DOWN]: Move Selection | [ENTER] Select Spell | [ESCAPE]: Cancel --", 38);
    }
    ...
    protected Screen use(Spell spell) {
        if (spell.isSelfCast()) {
            ...
            return new SpellTargetingScreen(
                    player, String.format("Cast %s at?", spell.name()), spell, null);
        }
