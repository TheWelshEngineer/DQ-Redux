...
public class CreatureFactory {
    ...
    public Player newPlayer(
            FieldOfView fov, NotificationHistory notificationsHandle, PlayerBuildDetails details) {
        // world, name, glyph, color, maxHP 20, maxMana, base armorclass, strength, dexterity,
        // intelligence, visionRadius, inventorySize) {
        Player player =
                new Player(
                        objectFactory.world,
                        "Player",
        ...
        switch (details.playerClass) {
            ...
            case MAGE:
                ...
                Item startWandMage =
                        objectFactory.itemFactory.newMagicMissileWand(0, player, false);
            ...
            case WITCH:
                ...
                Item startWandWitch_1 =
                        objectFactory.itemFactory.newChainLightningWand(0, player, false);
                player.learnNameQuiet(startWandWitch_1);
                player.inventory().add(startWandWitch_1);
                player.equipToQuickslot(startWandWitch_1, 1);
                Item startWandWitch_2 =
                        objectFactory.itemFactory.newAcidBlastWand(0, player, false);
            ...
            case PALADIN:
                ...
                Item startWandPaladin =
                        objectFactory.itemFactory.newBladsWardWand(0, player, false);
        ...
        if (details.ancestry == PlayerAncestry.DRAGONBORN) {
            ...
            if (details.playerClass == PlayerClass.MAGE
                    || details.playerClass == PlayerClass.PALADIN) {
    ...
    public Creature newFungus(int depth, boolean addToWorld) {
        Creature fungus =
                new Fungus(objectFactory, "Fungus", 'f', ExtendedAsciiPanel.green, 100, depth);
        if (addToWorld) {
    ...
    public Creature newSkeletonElectromancer(int depth, Creature player, boolean addToWorld) {
        Creature skeleton =
                new SkeletonElectromancer(
                        objectFactory,
                        player,
                        "Skeleton Electromancer",
                        'k',
    ...
    public Creature newThundercloudSlimeling(int depth, Creature player, boolean addToWorld) {
        Creature slimeling =
                new ThundercloudSlimeling(
                        objectFactory,
                        player,
                        "Thundercloud Slimeling",
                        's',
                        ExtendedAsciiPanel.paralyzed,
                        503,
                        depth);
    ...
    public Creature newCloaker(int depth, Creature player, boolean addToWorld) {
        Creature cloaker =
                new Cloaker(
                        ...
                        "Cloaker",
                        'c',
                        ExtendedAsciiPanel.brightBlack,
                        800,
                        depth);
        if (addToWorld) {
